...
class Traj:
    ...
    def __init__(self, u, k0, kN, Qc):
        ...
        covar_row = np.array(covar_row).transpose(0, 3, 1, 4, 2).reshape(self.N+1, self.X, self.X)
        
        temp = np.linalg.pinv((self.covar_func(self.N, self.N) + self.kN).transpose(2, 0, 3, 1).reshape(self.X, self.X))
        temp = covar_row.reshape((self.N+1) * self.X, self.X) @ temp
        self.temp = temp
        temp = temp @ covar_row.transpose(1, 0, 2).reshape(self.X, (self.N+1) * self.X)
        ...
        qinvV = np.kron(self.qinv, np.eye(self.V)).reshape(self.D, self.V, self.D, self.V).transpose(1, 3, 0, 2)
        qinvtemp = [np.linalg.pinv(self.k0)] + [qinvV for i in range(self.N+1)] + [np.linalg.pinv(self.kN)]
    ...
    def setQc(self, Qc): raise NotImplementedError()
    
    def at(self, t):
        assert self.N >= 1 and 0 <= t and t <= self.N
        return self.state_func(t)
        
    def optimize(self, grads_func, epochs=10000, lr=1e-4, tradeoff=0, dt=0.02):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0: util.plot_traj_int(self.at, self.sstates, self.Qc, 0, self.N)

            istates = (self.M @ self.sstates.reshape((self.N+1) * self.D, self.V)).reshape(self.N * self.n_ip + self.N + 1, self.D, self.V)
            g_up = grads_func(istates).reshape((self.N * self.n_ip + self.N + 1) * self.D, self.V)
            grads = (self.Mt @ g_up).reshape((self.N+1) * self.X, 1)
            # mean = self.MEAN + (self.temp @ (self.u[-1]-self.sstates[-1]).reshape(self.X, 1)).reshape(self.N+1, self.D, self.V)
            # dist_grads = tradeoff*self.Kinv@(self.sstates-mean).reshape(*grads.shape)
            
            
            
            
            
            self.sstates -= lr * (self.covar @ grads).reshape(*self.sstates.shape)